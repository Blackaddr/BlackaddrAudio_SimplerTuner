...
void Tuner::disable()
{
    ...
    //if (tunerManagerPtr) { tunerManagerPtr->setTunerMode(false); }
...
void Tuner::update(void)
{
    ...
    if (!inputAudioBlock) {  // EFX level bypass
        ...
        sendReports(-50.0f, static_cast<unsigned>(TunerNote::NO_DETECT));
        return;
    ...
    if (m_tunerEnabled) {
        ...
        if (updated) {
            ...
            sendReports(m_tunercents, noteNearest);
            m_signalDetect = true;
            m_skipUpdateCount = 0;
...
void Tuner::sendReports(float tunerCents, unsigned tunerNote)
{
    ...
    sendValueReport(TunerCents_e, tunerCents);
    sendValueReport(TunerNote_e, tunerNote);
...
void Tuner::bypass(bool value)
{
    ...
    if (tunerManagerPtr) { tunerManagerPtr->setTunerMode(m_tunerEnabled && m_enable); }
}
